...
const rescue_time_1 = require("./api/rescuetime");
...
(async () => {
  if (cosmic_1.config("daily").includes("spotify")) await spotify_1.daily();
  ...
  if (cosmic_1.config("daily").includes("wakatime")) await wakatime_1.daily();
  if (cosmic_1.config("daily").includes("lastFm")) await last_fm_1.daily();
  if (cosmic_1.config("daily").includes("clockify")) await clockify_1.daily();
  if (cosmic_1.config("daily").includes("googleFit"))
    await google_fit_1.daily();
  if (cosmic_1.config("daily").includes("ouraRing")) await oura_ring_1.daily();
  if (cosmic_1.config("daily").includes("goodreads")) await goodreads_1.daily();
})();
