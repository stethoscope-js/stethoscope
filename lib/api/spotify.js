...
const api = new spotify_web_api_node_1.default({
  clientId: cosmic_1.config("spotifyClientId") ?? "example",
  clientSecret: cosmic_1.config("spotifyClientSecret") ?? "example",
  redirectUri:
    cosmic_1.config("spotifyCallbackUrl") ?? "http://localhost:3000/callback",
  accessToken: cosmic_1.config("spotifyAccessToken") ?? "example",
  refreshToken: cosmic_1.config("spotifyRefreshToken") ?? "example",
});
exports.daily = async () => {
  ...
  const libraryItems = exports
    .cleanSpotifyTracksResponse(library.body.items.map((item) => item.track))
    .map((item, index) => ({
      ...item,
  ...
  const shortTermTopTracks = exports.cleanSpotifyTracksResponse(
  ...
  const mediumTermTopTracks = exports.cleanSpotifyTracksResponse(
  ...
  const longTermTopTracks = exports.cleanSpotifyTracksResponse(
  ...
  const shortTermTopArtists = exports.cleanSpotifyArtistsResponse(
  ...
  const mediumTermTopArtists = exports.cleanSpotifyArtistsResponse(
  ...
  const longTermTopArtists = exports.cleanSpotifyArtistsResponse(
