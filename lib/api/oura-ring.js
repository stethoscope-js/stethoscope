...
const updateOuraDailyData = async (date) => {
    ...
    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "daily", "weight", dayjs_1.default(formattedDate).format("YYYY"), dayjs_1.default(formattedDate).format("MM"), dayjs_1.default(formattedDate).format("DD"), "summary.json"), JSON.stringify({ weight: healthData.weight }, null, 2));
    ...
    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "daily", "sleep", dayjs_1.default(formattedDate).format("YYYY"), dayjs_1.default(formattedDate).format("MM"), dayjs_1.default(formattedDate).format("DD"), "summary.json"), JSON.stringify(sleepData.sleep, null, 2));
    ...
    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "daily", "readiness", dayjs_1.default(formattedDate).format("YYYY"), dayjs_1.default(formattedDate).format("MM"), dayjs_1.default(formattedDate).format("DD"), "summary.json"), JSON.stringify(readinessData.readiness, null, 2));
    ...
    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "daily", "activity", dayjs_1.default(formattedDate).format("YYYY"), dayjs_1.default(formattedDate).format("MM"), dayjs_1.default(formattedDate).format("DD"), "summary.json"), JSON.stringify(activityData.activity, null, 2));
};
exports.daily = async () => {
...
exports.summary = async () => {
    const types = await fs_extra_1.readdir(path_1.join(".", "data", "health", "oura-ring", "daily"));
    for await (const dataType of types.filter((type) => type !== "weight")) {
        const yearMonths = {};
        const weeks = {};
        const years = await fs_extra_1.readdir(path_1.join(".", "data", "health", "oura-ring", "daily", dataType));
        for await (const year of years) {
            const months = await fs_extra_1.readdir(path_1.join(".", "data", "health", "oura-ring", "daily", dataType, year));
            for await (const month of months) {
                const days = await fs_extra_1.readdir(path_1.join(".", "data", "health", "oura-ring", "daily", dataType, year, month));
                for await (const day of days) {
                    const data = await fs_extra_1.readJson(path_1.join(".", "data", "health", "oura-ring", "daily", dataType, year, month, day, "summary.json"));
                    let sum = 0;
        ...
        for await (const year of Object.keys(weeks)) {
            for await (const week of [
                ...
                if (dayjs_1.default(`${year}-06-06`).week(week).startOf("week").isBefore(dayjs_1.default())) {
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "weekly", dataType, year, week.toString(), "summary.json"), JSON.stringify(days, null, 2));
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "weekly", dataType, year, week.toString(), "graph.png"), image);
                }
            }
        }
        ...
        for await (const year of Object.keys(yearMonths)) {
            ...
            for await (const month of [...Array(12).keys()].map((i) => i + 1)) {
                if (dayjs_1.default(`${year}-${common_1.zero(month.toString())}-${dayjs_1.default(month).daysInMonth()}`).isBefore(dayjs_1.default())) {
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "monthly", dataType, year, month.toString(), "summary.json"), JSON.stringify(monthly, null, 2));
                    const image = await canvasRenderService.renderToBuffer({
                        type: "bar",
                        data: {
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "oura-ring", "monthly", dataType, year, month.toString(), "graph.png"), image);
                }
            }
            await common_1.write(path_1.join(".", "data", "health", "oura-ring", "yearly", dataType, year, "summary.json"), JSON.stringify(yearly, null, 2));
            const image = await canvasRenderService.renderToBuffer({
                type: "bar",
                data: {
            ...
            await common_1.write(path_1.join(".", "data", "health", "oura-ring", "yearly", dataType, year, "graph.png"), image);
        }
