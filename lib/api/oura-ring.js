...
const isoWeeksInYear_1 = __importDefault(
  require("dayjs/plugin/isoWeeksInYear")
);
...
const updateOuraDailyData = async (date) => {
  ...
  const { data: healthData } = await axios_1.default.get(
    `https://api.ouraring.com/v1/userinfo?access_token=${cosmic_1.config(
      "ouraPersonalAccessToken"
    )}`
  );
  await common_1.write(
    path_1.join(
      ".",
      "data",
      "oura-weight",
      "daily",
      dayjs_1.default(formattedDate).format("YYYY"),
      dayjs_1.default(formattedDate).format("MM"),
      dayjs_1.default(formattedDate).format("DD"),
      "sessions.json"
    ),
    JSON.stringify({ weight: healthData.weight }, null, 2)
  );
  console.log("Oura: Added summary data");
  const { data: sleepData } = await axios_1.default.get(
    `https://api.ouraring.com/v1/sleep?start=${formattedDate}&end=${formattedDate}&access_token=${cosmic_1.config(
      "ouraPersonalAccessToken"
    )}`
  );
  ...
  const { data: readinessData } = await axios_1.default.get(
    `https://api.ouraring.com/v1/readiness?start=${formattedDate}&end=${formattedDate}&access_token=${cosmic_1.config(
      "ouraPersonalAccessToken"
    )}`
  );
  ...
  const { data: activityData } = await axios_1.default.get(
    `https://api.ouraring.com/v1/activity?start=${formattedDate}&end=${formattedDate}&access_token=${cosmic_1.config(
      "ouraPersonalAccessToken"
    )}`
  );
...
exports.daily = async () => {
  ...
  for await (const day of [0, 1, 2, 3, 4]) {
    await updateOuraDailyData(dayjs_1.default().subtract(day, "day").toDate());
...
exports.summary = async () => {
  for await (const key of [
    ...
    for await (const category of [
      ...
      if (
        (await fs_extra_1.pathExists(
          path_1.join(".", "data", category, "daily")
        )) &&
        (
          await fs_extra_1.lstat(path_1.join(".", "data", category, "daily"))
        ).isDirectory()
      ) {
        const years = (
          await fs_extra_1.readdir(path_1.join(".", "data", category, "daily"))
        ).filter((i) => /^\d+$/.test(i));
        const yearData = {};
        const weeklyData = {};
        for await (const year of years) {
          ...
          const months = (
            await fs_extra_1.readdir(
              path_1.join(".", "data", category, "daily", year)
            )
          ).filter((i) => /^\d+$/.test(i));
          for await (const month of months) {
            ...
            [
              ...Array(
                dayjs_1.default(`${year}-${month}-10`).daysInMonth()
              ).keys(),
            ]
              .slice(1)
            ...
            const days = (
              await fs_extra_1.readdir(
                path_1.join(".", "data", category, "daily", year, month)
              )
            ).filter((i) => /^\d+$/.test(i));
            for await (const day of days) {
              let json = await fs_extra_1.readJson(
                path_1.join(
                  ".",
                  "data",
                  category,
              ...
              Object.keys(dailyData).forEach((key) => {
                const weekNumber = dayjs_1
                  .default(`${year}-${month}-${key}`)
                  .week();
...
exports.legacy = async () => {
  ...
  const pool = new es6_promise_pool_1.default(async () => {
    ...
    if (dayjs_1.default().diff(date, "day") === 0) return null;
    count++;
