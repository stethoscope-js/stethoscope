...
const oauth2Client = new googleapis_1.google.auth.OAuth2(cosmic_1.config("googleFitClientId") ?? "example", cosmic_1.config("googleFitClientSecret") ?? "example", "https://developers.google.com/oauthplayground");
...
exports.daily = async () => {
    ...
    for await (const day of [0, 1, 2, 3, 4]) {
        await updateGoogleFitDailyData(dayjs_1.default().subtract(day, "day").toDate());
            ...
            for await (const year of Object.keys(weeklyData)) {
                for await (const week of Object.keys(weeklyData[year])) {
                    if (Object.keys(weeklyData[year][week]).length &&
                        ...
                        await common_1.write(path_1.join(".", "data", `google-fit-${category}`, "summary", "weeks", year, `${week}.json`), JSON.stringify(weeklyData[year][week], null, 2));
                }
            }
        }
    }
