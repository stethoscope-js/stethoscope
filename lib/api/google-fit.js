        ...
        for await (const year of years) {
            ...
            for await (const month of months) {
                ...
                for await (const day of days) {
                    const _data = await fs_extra_1.readJson(path_1.join(".", "data", "health", "google-fit", "daily", dataType, year, month, day, "sessions.json"));
                    /**
        ...
        for await (const year of Object.keys(weeks)) {
            for await (const week of [
                ...
                if (dayjs_1.default(`${year}-06-06`).week(week).startOf("week").isBefore(dayjs_1.default())) {
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "google-fit", "weekly", dataType, year, week.toString(), "summary.json"), JSON.stringify(days, null, 2));
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "google-fit", "weekly", dataType, year, week.toString(), "graph.png"), image);
                }
            }
        }
        ...
        for await (const year of Object.keys(yearMonths)) {
            ...
            for await (const month of [...Array(12).keys()].map((i) => i + 1)) {
                if (dayjs_1.default(`${year}-${common_1.zero(month.toString())}-${dayjs_1.default(month).daysInMonth()}`).isBefore(dayjs_1.default())) {
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "google-fit", "monthly", dataType, year, month.toString(), "summary.json"), JSON.stringify(monthly, null, 2));
                    const image = await canvasRenderService.renderToBuffer({
                        type: "bar",
                        data: {
                    ...
                    await common_1.write(path_1.join(".", "data", "health", "google-fit", "monthly", dataType, year, month.toString(), "graph.png"), image);
                }
            }
            await common_1.write(path_1.join(".", "data", "health", "google-fit", "yearly", dataType, year, "summary.json"), JSON.stringify(yearly, null, 2));
            const image = await canvasRenderService.renderToBuffer({
                type: "bar",
                data: {
            ...
            await common_1.write(path_1.join(".", "data", "health", "google-fit", "yearly", dataType, year, "graph.png"), image);
        }
