...
const oauth2Client = new googleapis_1.google.auth.OAuth2(
  cosmic_1.config("googleFitClientId") ?? "example",
  cosmic_1.config("googleFitClientSecret") ?? "example",
  "https://developers.google.com/oauthplayground"
);
oauth2Client.setCredentials({
  access_token: cosmic_1.config("googleFitAccessToken") ?? "example",
  refresh_token: cosmic_1.config("googleFitRefreshToken") ?? "example",
});
const fitness = googleapis_1.google.fitness({
  version: "v1",
  auth: oauth2Client,
});
...
exports.daily = async () => {
  ...
  for await (const day of [0, 1, 2, 3, 4]) {
    await updateGoogleFitDailyData(
      dayjs_1.default().subtract(day, "day").toDate()
    );
  ...
  for await (const category of [
    ...
    if (
      (await fs_extra_1.pathExists(
        path_1.join(".", "data", `google-fit-${category}`, "daily")
      )) &&
      (
        await fs_extra_1.lstat(
          path_1.join(".", "data", `google-fit-${category}`, "daily")
        )
      ).isDirectory()
    ) {
      ...
      for await (const year of years) {
        ...
        const months = (
          await fs_extra_1.readdir(
            path_1.join(".", "data", `google-fit-${category}`, "daily", year)
          )
        ).filter((i) => /^\d+$/.test(i));
            ...
            if (Array.isArray(json)) {
                  ...
                  .sort(
                    (a, b) =>
                      dayjs_1.default(a.startTime).unix() -
                        dayjs_1.default(b.startTime).unix() ||
                      dayjs_1.default(a.endTime).unix() -
                        dayjs_1.default(b.endTime).unix()
                  )
                  .reduce((r, a) => {
                    const last = r[r.length - 1] || [];
                    if (
                      dayjs_1.default(last.startTime).unix() <=
                        dayjs_1.default(a.startTime).unix() &&
                      dayjs_1.default(a.startTime).unix() <=
                        dayjs_1.default(last.endTime).unix()
                    ) {
                      if (
                        dayjs_1.default(last.endTime).unix() <
                        dayjs_1.default(a.endTime).unix()
              ...
              json.forEach((record) => {
                if (record.startTime && record.endTime) {
                  dailySum += dayjs_1
                    .default(record.endTime)
                    .diff(record.startTime, "second");
