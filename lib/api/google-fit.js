...
exports.summary = async () => {
    const healthCategories = (await fs_extra_1.readdir(path_1.join(".", "data", "health"))).filter((category) => ["readiness", "activity", "weight", "oura-sleep"].includes(category));
    for await (const category of healthCategories) {
        // Find all items that have daily
        if ((await fs_extra_1.pathExists(path_1.join(".", "data", "health", category, "daily"))) &&
            (await fs_extra_1.lstat(path_1.join(".", "data", "health", category, "daily"))).isDirectory()) {
            const years = (await fs_extra_1.readdir(path_1.join(".", "data", "health", category, "daily"))).filter((i) => /^\d+$/.test(i));
            const yearData = {};
            for await (const year of years) {
                ...
                const months = (await fs_extra_1.readdir(path_1.join(".", "data", "health", category, "daily", year))).filter((i) => /^\d+$/.test(i));
                for await (const month of months) {
                    ...
                    const days = (await fs_extra_1.readdir(path_1.join(".", "data", "health", category, "daily", year, month))).filter((i) => /^\d+$/.test(i));
                    for await (const day of days) {
                        let json = await fs_extra_1.readJson(path_1.join(".", "data", "health", category, "daily", year, month, day, "sessions.json"));
                        let dailySum = 0;
                    ...
                    if (Object.keys(dailyData).length)
                        await common_1.write(path_1.join(".", "data", category, "summary", "days", year, `${month}.json`), JSON.stringify(dailyData, null, 2));
                    if (monthlySum)
                        monthlyData[parseInt(month)] = monthlySum;
                }
                if (Object.keys(monthlyData).length)
                    await common_1.write(path_1.join(".", "data", category, "summary", "months", `${year}.json`), JSON.stringify(monthlyData, null, 2));
                if (yearlySum)
                    yearData[parseInt(year)] = yearlySum;
            }
            if (Object.keys(yearData).length)
                await common_1.write(path_1.join(".", "data", category, "summary", "years.json"), JSON.stringify(yearData, null, 2));
        }
    }
};
