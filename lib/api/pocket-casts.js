...
const pocketCasts = new pocketcasts_1.default(
  cosmic_1.config("pocketCastsUsername") ?? "example",
  cosmic_1.config("pocketCastsPassword") ?? "example"
);
exports.daily = async () => {
  ...
  await common_1.write(
    path_1.join(".", "data", "pocket-casts-podcasts", "library.json"),
    JSON.stringify(podcasts, null, 2)
  );
  console.log("Pocket Casts: Added library");
  let items = [];
  try {
    const years = await fs_extra_1.readdir(
      path_1.join(".", "data", "pocket-casts-podcasts", "daily")
    );
    const months = await fs_extra_1.readdir(
      path_1.join(
        ".",
        "data",
        "pocket-casts-podcasts",
        "daily",
        common_1.zero(Math.max(...years.map(parseInt)).toString())
      )
    );
    const days = await fs_extra_1.readdir(
      path_1.join(
        ".",
        "data",
        "pocket-casts-podcasts",
        "daily",
        common_1.zero(Math.max(...years.map(parseInt)).toString()),
        common_1.zero(Math.max(...months.map(parseInt)).toString())
      )
    );
    items = await fs_extra_1.readJson(
      path_1.join(
        ".",
        "data",
        "pocket-casts-podcasts",
        ...
        common_1.zero(Math.max(...years.map(parseInt)).toString()),
        common_1.zero(Math.max(...months.map(parseInt)).toString()),
        common_1.zero(Math.max(...days.map(parseInt)).toString()),
