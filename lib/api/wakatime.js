...
exports.daily = async () => {
    ...
    for await (const day of [0, 1, 2, 3, 4]) {
        await updateWakatimeDailyData(dayjs_1.default().subtract(day, "day").toDate());
        ...
        for await (const year of Object.keys(weeklyData)) {
            for await (const week of Object.keys(weeklyData[year])) {
                if (Object.keys(weeklyData[year][week]).length &&
                    ...
                    await common_1.write(path_1.join(".", "data", "wakatime-time-tracking", "summary", "weeks", year, `${week}.json`), JSON.stringify(weeklyData[year][week], null, 2));
            }
        }
    }
};
