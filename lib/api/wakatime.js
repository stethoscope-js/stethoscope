...
const client = new wakatime_client_1.WakaTimeClient(
  cosmic_1.config("wakatimeApiKey") ?? "example"
);
...
exports.summary = async () => {
  if (
    (await fs_extra_1.pathExists(
      path_1.join(".", "data", "wakatime-time-tracking", "daily")
    )) &&
    (
    ...
    for await (const year of years) {
      ...
      const months = (
        await fs_extra_1.readdir(
          path_1.join(".", "data", "wakatime-time-tracking", "daily", year)
        )
      ).filter((i) => /^\d+$/.test(i));
