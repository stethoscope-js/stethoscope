...
const getTimeData = async (date) => {
    ...
    for await (const key of Object.keys(itemsByDate)) {
        await common_1.write(path_1.join(".", "data", "clockify-time-tracking", "daily", key, "time-entries.json"), JSON.stringify(itemsByDate[key], null, 2));
    }
...
exports.daily = async () => {
    ...
    for await (const day of [0, 1, 2, 3, 4]) {
        await getTimeData(dayjs_1.default().subtract(day, "day").toDate());
