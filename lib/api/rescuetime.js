...
const updateRescueTimeDailyData = async (date) => {
  ...
  const topCategories = (
    await axios_1.default.get(
      `https://www.rescuetime.com/anapi/data?format=json&key=${cosmic_1.config(
        "rescuetimeApiKey"
...
exports.daily = async () => {
  ...
  for await (const day of [0, 1, 2, 3, 4]) {
    await updateRescueTimeDailyData(
...
exports.summary = async () => {
  if (
    (await fs_extra_1.pathExists(
      path_1.join(".", "data", "rescuetime-time-tracking", "daily")
    )) &&
    (
    ...
    for await (const file of ["top-categories.json", "top-overview.json"]) {
      ...
      for await (const year of years) {
        ...
        const months = (
          await fs_extra_1.readdir(
            path_1.join(".", "data", "rescuetime-time-tracking", "daily", year)
          )
        ).filter((i) => /^\d+$/.test(i));
        for await (const month of months) {
          ...
          for await (const day of days) {
            let json = [];
            try {
              json = await fs_extra_1.readJson(
                path_1.join(
                  ".",
                  "data",
                  "rescuetime-time-tracking",
