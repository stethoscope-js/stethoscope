        ...
        for await (const year of years) {
            ...
            for await (const month of months) {
                ...
                for await (const day of days) {
                    let json = await fs_extra_1.readJson(path_1.join(".", "data", "rescuetime-time-tracking", "daily", year, month, day, "top-categories.json"));
                    let dailySum = 0;
                ...
                if (Object.keys(dailyData).length)
                    await common_1.write(path_1.join(".", "data", "rescuetime-time-tracking", "summary", "days", year, `${month}.json`), JSON.stringify(dailyData, null, 2));
                if (monthlySum)
                    monthlyData[parseInt(month)] = monthlySum;
            }
            if (Object.keys(monthlyData).length)
                await common_1.write(path_1.join(".", "data", "rescuetime-time-tracking", "summary", "months", `${year}.json`), JSON.stringify(monthlyData, null, 2));
            if (yearlySum)
                yearData[parseInt(year)] = yearlySum;
        }
        if (Object.keys(yearData).length)
            await common_1.write(path_1.join(".", "data", "rescuetime-time-tracking", "summary", "years.json"), JSON.stringify(yearData, null, 2));
    }
};
